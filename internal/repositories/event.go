...
func (r *EventRepository) Create(req *models.EventCreateRequest, organizerID int) (*models.Event, error) {
	...
	// Prepare image data - use NULL/zero values if no image
	var imageURL, imageKey, imageFormat interface{}
	...
	if req.ImageURL != "" && req.ImageKey != "" {
		...
		imageURL = nil
		imageKey = nil
		imageSize = 0  // Must be 0, not NULL
		imageFormat = nil
		imageWidth = 0  // Must be 0, not NULL
		imageHeight = 0  // Must be 0, not NULL
		imageUploadedAt = nil
	}

	var imageURLScan, imageKeyScan, imageFormatScan sql.NullString
	var imageSizeScan sql.NullInt64
	var imageWidthScan, imageHeightScan sql.NullInt32
	var imageUploadedAtScan sql.NullTime
	...
	if imageURLScan.Valid {
		event.ImageURL = imageURLScan.String
	}
	if imageKeyScan.Valid {
		event.ImageKey = imageKeyScan.String
	}
	if imageFormatScan.Valid {
		event.ImageFormat = imageFormatScan.String
	}
	if imageSizeScan.Valid {
		event.ImageSize = imageSizeScan.Int64
	}
	if imageWidthScan.Valid {
		event.ImageWidth = int(imageWidthScan.Int32)
	}
	if imageHeightScan.Valid {
		event.ImageHeight = int(imageHeightScan.Int32)
	}
	if imageUploadedAtScan.Valid {
		event.ImageUploadedAt = &imageUploadedAtScan.Time
	}
...
func (r *EventRepository) GetByOrganizer(organizerID int) ([]*models.Event, error) {
	...
	for rows.Next() {
		event := &models.Event{}
		var imageURL, imageKey, imageFormat sql.NullString
		var imageSize sql.NullInt64
		var imageWidth, imageHeight sql.NullInt32
		var imageUploadedAt sql.NullTime
		...
		if imageKey.Valid {
			event.ImageKey = imageKey.String
		}
		if imageFormat.Valid {
			event.ImageFormat = imageFormat.String
		}
		if imageSize.Valid {
			event.ImageSize = imageSize.Int64
		}
		if imageWidth.Valid {
			event.ImageWidth = int(imageWidth.Int32)
		}
		if imageHeight.Valid {
			event.ImageHeight = int(imageHeight.Int32)
		}
		if imageUploadedAt.Valid {
			event.ImageUploadedAt = &imageUploadedAt.Time
		}
...
// scanEvent is a helper function to scan event data from database rows
